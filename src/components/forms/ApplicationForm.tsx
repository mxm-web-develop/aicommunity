...
export default function ApplicationForm() {
  ...
  const handleArrayInput = (field: 'keywords' | 'awards', value: string) => {
    if (value.trim()) {
      ...
      // 过滤掉已存在的项
      const uniqueItems = newItems.filter(item => !formData[field].includes(item));
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-lg">
      ...
      <div className="mb-4">
        ...
        <input
          ...
          onChange={(e) => setFormData({ ...formData, gientechType: e.target.value })}
          placeholder="请输入应用类型"
        />
      </div>
      ...
      <div className="mb-4">
        ...
        <Select
          ...
          onChange={(selected) => {
            const selectedValues = (selected as any[]).map(option => option.value);
            setFormData({
              ...formData,
